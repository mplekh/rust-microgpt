...
impl Tape {
    ...
    pub fn truncate(&mut self, new_len: usize) {
        ...
        self.arity.truncate(new_len);
    }

    pub fn val(&mut self, data: f64) -> V {
        self.push(data, [0, 0], [0.0, 0.0], 0)
    }
...
pub fn add(t: &mut Tape, a: V, b: V) -> V {
    ...
    t.push(data, [a, b], [1.0, 1.0], 2)
}

pub fn mul(t: &mut Tape, a: V, b: V) -> V {
    let data = t.data[a] * t.data[b];
    t.push(data, [a, b], [t.data[b], t.data[a]], 2)
}

pub fn neg(t: &mut Tape, a: V) -> V {
    let data = -t.data[a];
    t.push(data, [a, 0], [-1.0, 0.0], 1)
}
...
pub fn div(t: &mut Tape, a: V, b: V) -> V {
    let data = t.data[a] / t.data[b];
    t.push(data, [a, b], [1.0 / t.data[b], - t.data[a] / (t.data[b] * t.data[b])], 2)
}

pub fn mul_const(t: &mut Tape, a: V, c: f64) -> V {
    let data = t.data[a] * c;
    t.push(data, [a, usize::MAX], [c, 0.0], 1)
}

pub fn pow(t: &mut Tape, a: V, p: f64) -> V {
    let a_data = t.data[a];
    let data = a_data.powf(p);
    t.push(data, [a, usize::MAX], [p * a_data.powf(p - 1.0), 0.0], 1)
}

pub fn exp(t: &mut Tape, a: V) -> V {
    let a_data = t.data[a];
    let e = a_data.exp();
    t.push(e, [a, usize::MAX], [e, 0.0], 1)
}

pub fn log(t: &mut Tape, a: V) -> V {
    let a_data = t.data[a];
    let data = a_data.ln();
    t.push(data, [a, usize::MAX], [1.0 / a_data, 0.0], 1)
}

pub fn relu(t: &mut Tape, a: V) -> V {
    let x = t.data[a];
    let d = if x > 0.0 { 1.0 } else { 0.0 };
    t.push(x.max(0.0), [a, usize::MAX], [d, 0.0], 1)
}
